-- ============================================================================
-- 0056 — WARD FORECAST (AI: LOS + 7-day occupancy)
--  * avg length-of-stay per ward_type from completed admissions.
--  * per active admission: elapsed stay + projected discharge date.
--  * 7-day occupancy projection = current census remaining + expected new
--    admissions (rolling 28-day admit rate per ward_type).
-- ============================================================================
...
BEGIN
  ...
  SELECT COALESCE(jsonb_object_agg(w.ward_type, round(avg(EXTRACT(EPOCH FROM (a.discharged_at - a.admitted_at)) / 86400.0)::numeric, 1)), '{}'::jsonb)
    INTO v_los

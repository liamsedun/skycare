-- ============================================================================
-- 0054 — LAB ANALYTICS: items count must count only matched invoice items
-- (the LEFT LATERAL projects one row per window month even with zero
-- matches, so COUNT(*) over-counted; COUNT(tx.invoice_id) counts matches.)
-- ============================================================================
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    FROM (
      SELECT to_char(mo, 'YYYY-MM') AS month,
             ...
             COUNT(tx.invoice_id)::bigint AS items

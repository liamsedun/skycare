...
// booking, no duty_roster clash, clinical staff need verified credentials.
...
export const POST = withStaff(async (req, ctx) => {
  ...
  if (error) {
    if (/CREDENTIAL_REQUIRED/.test(error.message)) {
      throw new ValidationError("Clinical staff member has no verified, unexpired credential");
    }

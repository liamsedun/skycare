...
- **Internal mail** â€” `/app/mail` (ADMIN): tabs Inbox/Sent/Compose; broadcast-to-staff; sidebar unread badge via `UnreadMailBadge` polling `/api/mail/unread-count` every 30s. APIs (all `withStaff`): `GET/POST /api/mail/inbox`, `GET /api/mail/sent`, `GET /api/mail/recipients` (staff/patients), `PUT /api/mail/[recipientRowId]/read`. Inserts into `internal_messages`+`internal_message_recipients`, notifies recipients, tenant-validates recipients.
- **Chats** â€” `/app/chats` (ADMIN): staffâ†"patient messaging; two-pane list+thread, presence dots, directory of portal patients for new chats, 5s message polling. APIs: `GET/POST /api/chats`, `GET/POST /api/chats/[chatId]/messages`, `POST /api/chat-presence` heartbeat. Patient API rejects patient_api role. Notifies patient's user_id on inbound staff message.
